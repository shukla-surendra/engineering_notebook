# engineering_notebook

Personal notes on algorithms, ML/LLM systems, distributed-systems design, and security
engineering, organized as four independent MkDocs sites:

- **`dsa_prep/`** — Algorithms & Data Structures. `make dsa-serve` / `make dsa-build`.
- **`system_design/`** — ML/LLM Systems Design. `make sd-serve` / `make sd-build`.
- **`staff_system_design/`** — General Distributed Systems Design. `make staff-serve` / `make staff-build`.
- **`security/`** — Cybersecurity, LLM Security & Cloud/MLOps/LLMOps Security. `make security-serve` / `make security-build`.

Run `make help` for the full command list, or `make mkdocs` to build and serve all four
together.
